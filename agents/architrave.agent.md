...
1. **Understand the specs.** Restate the request + source-of-truth (Storybook/`config.designMap` + the platform pack) as a **numbered, testable acceptance-criteria checklist** (BDD: behavior before build). Confirm ambiguities with the user. Use Explore for fast context, and Product Research when shipped product precedent or domain workflow evidence is needed. When `config.designSource.mcp` is set, **open with `list-all-documentation`** (then `get-documentation` on the components you'll touch) to ground in real components before writing the checklist.
...
1. **Understand + classify.** Restate the request as testable acceptance criteria; decide UI/app-only / backend-only / full-stack; scale the crew. Use Explore for fast recon of the `solution` + ADRs.
...
1. **Classify the runtime question.** What claim needs runtime evidence: health, logs, ingress, deployed image, version drift, feature availability, or post-deploy behavior?
