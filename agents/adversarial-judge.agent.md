...
2. **Reason first, then judge** (improves judgment, reduces bias): for each rubric dimension, enumerate concrete failure scenarios and check the proposal/implementation against them — spec conformance, design-language conformance, platform/accessibility conformance, contract conformance, migration/idempotency, IaC plan-only safety, adversarial/edge states, security, tokens/reconciliation, tests, verification.
