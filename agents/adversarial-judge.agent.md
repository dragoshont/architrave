...
Apply the canonical rubric: `gates/rubric.md`. Read `uikit.config.json` to learn the `platform`, `stack`, `designSource`, `designMap`, `tokens`, and any `backend` / `iac` blocks you are judging against. Load the matching platform knowledge pack (`knowledge/apple.md` | `microsoft.md` | `web.md`) and `knowledge/backend.md` when backend/infra is in scope. When `config.designSource.mcp` is set, verify the design‑language claim against `get-documentation` for the reproduced component (its real props/stories), not assumptions.
...
- DO NOT pass anything that reinvents an existing Storybook/`config.designMap` component, claims a capability the app can't truthfully perform, hard-codes values a token should own, skips visible intake/tournament for non-trivial work, or violates the repo's stated policy — those are automatic FAIL/Blocker or REVISE/Major depending on blast radius.
...
2. **Reason first, then judge** (improves judgment, reduces bias): for each rubric dimension, enumerate concrete failure scenarios and check the proposal/implementation against them — visible intake, tournament/options quality, recommended plan, spec conformance, design-language conformance, platform/accessibility conformance, contract conformance, migration/idempotency, IaC plan-only safety, adversarial/edge states, security, tokens/reconciliation, tests, verification.
