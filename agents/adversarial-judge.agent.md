...
- DO NOT accept a role label as execution evidence, a concrete model binding as universal policy, or a stronger model as a substitute for gates. Treat requested bindings and producer-reported selections as claims until host telemetry supports them.
...
2. **Reason first, then judge** (improves judgment, reduces bias): for each rubric dimension, enumerate concrete failure scenarios and check the proposal/implementation against them — visible intake, adaptive execution rationale and evidence, tournament/options quality, phase ledger and phase-boundary clarity, YAGNI ladder use, recommended plan, spec conformance, design-language conformance, platform/accessibility conformance, contract conformance, migration/idempotency, IaC plan-only safety, learning/audit trail, stale-fact validation, adversarial/edge states, security, tokens/reconciliation, tests, verification.
