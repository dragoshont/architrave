...
if (-not (Test-Path (Join-Path $Target 'uikit.config.json'))) {
  [Console]::Error.WriteLine("update: $Target has no uikit.config.json - run tools/install.ps1 first"); exit 1
}
...
Write-Host '  ok knowledge refreshed (apple/microsoft/web/backend/design-tokens)'
...
Write-Host 'Done. (uikit.config.json and .github/agents left untouched.)'
